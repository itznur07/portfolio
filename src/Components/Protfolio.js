...
const Card = ({ img, name, describe, tags, repolink, livelink }) => {
  return (
    <div className='md:w-80 w-72 h-auto py-5 bg-[#1d293a] rounded md:flex items-center justify-center cursor-pointer hover:bg-white group transition ease-linear duration-200 border border-[#0eb761] border-opacity-30'>
      <div className='flex flex-col items-center gap-y-5'>
        <span className='text-[#0eb761] text-7xl'>
          <img className="grayscale group-hover:grayscale-0 transition ease-linear duration-200" src={img} alt='' />
        </span>
        <h1 className='text-2xl text-[#fff] group-hover:text-black font-semibold'>
          {name}
        </h1>
        <p className='text-center text-sm text-[#ddd] group-hover:text-black'>
          {describe}
        </p>
        <span className='flex text-xl text-white group-hover:text-black  space-x-5'>
        ...
        <div className='text-center flex items-center space-x-5 mt-2'>
          {
            tags.map((item, i) => <span key={i} className='bg-[#636f81] px-3 py-1 rounded-md text-white'>
            {item}
            </span>)
          }
        </div>
